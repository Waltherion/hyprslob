...
ShellRoot {
    ...
    signal themesRequest(string choicesPath, string resultPath)
    IpcHandler {
        target: "hyprslob"
    ...
    Variants {
        ...
        PanelWindow {
            ...
            property real level2AnimW: win.hubActive === "launcher" ? win.launcherW
                                     : (win.hubActive === "dmenu" || win.hubActive === "menu" || win.hubActive === "wallpapers" || win.hubActive === "themes") ? (l2.item ? l2.item.implicitWidth : win.level2W)
            ...
            // ---- Theme grid (thumbnail picker over all themes; mirrors the wallpaper grid) ----
            property string thResultPath: ""
            property var thEntries: []
            FileView {
                id: thChoicesFile
                onLoaded: win.showThemePanel(win.parseThemeEntries(thChoicesFile.text()))
                onLoadFailed: win.showThemePanel([])
            }
            function parseThemeEntries(text) {
                const lines = (text || "").split("\n").filter(l => l.length > 0);
                return lines.map(l => {
                    const p = l.split("\t");
                    return { label: p[0], image: (p[1] || ""), colors: (p[2] ? p[2].split(",").filter(c => c.length) : []), dir: (p[3] || "") };
                });
            }
            function openThemes(choicesPath, resultPath) {
                win.thResultPath = resultPath;
                thChoicesFile.path = choicesPath;
                thChoicesFile.reload();
            }
            function showThemePanel(entries) {
                win.thEntries = entries;
                win.expandLevel = 1;
                win.hubActive = "themes";
            }
            function finishThemes(dir) {   // chosen theme dir name, or "" on cancel
                if (win.thResultPath.length) {
                    dmResultFile.path = win.thResultPath;
                    dmResultFile.setText(dir);
                    win.thResultPath = "";
                }
                win.expandLevel = 0;
            }
            ...
            Connections {
                ...
                function onThemesRequest(choicesPath, resultPath) {
                    if (!win.isFocused) return;
                    win.openThemes(choicesPath, resultPath);
                }
            }
